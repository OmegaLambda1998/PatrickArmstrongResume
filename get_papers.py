...
def get_papers(content):
    data = xmltodict.parse(content.decode("utf-8"))["activities:works"][
        "activities:group"
    ]
    ...
    def task(paper_data):
        ...
        if authors is None:
            print(
                f"Warning can't get authors for paper: '{paper['title']}', missing doi, bibcode or arxiv-id."
            )
            print(paper)
...
def get_args():
    ...
    parser.add_argument(
        "-id", "--orcid", help="Specify your orcid id", type=str, required=True
    )
    parser.add_argument(
        "-o", "--output", help="File to save papers to", type=str, default="papers.json"
    )
